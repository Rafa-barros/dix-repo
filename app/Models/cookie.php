	...
	class cookie{
		...
		*/
		public function newCookie($email){
			...
			$token = md5($new . $email);
			$conn = new Database();
			$conn->executeQuery('UPDATE  SET token = :TOKEN WHERE email = :EMAIL', array(
				':TOKEN' => $token,
				':EMAIL' => $email
			));
		...
		*/
		public function verifyCookie($token, $cUser){
			...
			$conn = new Database();
			$result = $conn->executeQuery('SELECT email, token FROM  WHERE email = :EMAIL AND token = :TOKEN LIMIT 1', array(
				':USER' => $email,
				':TOKEN' => $token
			));
			$res = $result->fetch();
		...
		*/
		public function deleteCookie($email){
			$conn = new Database();
			$conn->executeQuery('UPDATE  SET token = NULL WHERE email = :EMAIL', array(
				':EMAIL' => $email
			));
