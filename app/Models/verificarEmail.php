...
class verificarEmail{
	...
	private function verificaCredenciais(){
		$result = $this->conn->executeQuery('SELECT id, email, registerDate, codeAccess FROM codigoverificacao WHERE id = :ID AND codeAccess = :CODIGO AND email = :EMAIL LIMIT 1', array(
			':ID' => $this->id,
			':CODIGO' => $this->codigo,
			':EMAIL' => $this->email
		));
